...
const EMPTY_PROFILE = {
  ...
  email: "", phone: "", linkedin: "", github: "", portfolio: "", avatar: "",
}
...
export default function Perfil() {
  ...
  const [loading, setLoading] = useState(Boolean(getStoredProfileId()))
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setProfile({
        ...
        title: p.title ?? "", city: p.city ?? "", state: p.state ?? "",
        email: p.email ?? "", phone: p.phone ?? "", linkedin: p.linkedin ?? "",
        github: p.github ?? "", portfolio: p.portfolio ?? "", avatar: p.avatar_url ?? "",
      })
  ...
  function profilePayload() {
    return {
      ...
      title: profile.title, city: profile.city, state: profile.state,
  ...
  const expPayload = (e) => ({
    ...
    start_date: e.start_date || null, end_date: e.end_date || null, current: e.current,
  })
  const eduPayload = (e) => ({
    course: e.course, institution: e.institution,
    start_date: e.start_date || null, end_date: e.end_date || null, current: e.current,
  })
  ...
  async function handleSave() {
    ...
    try {
      if (isNew) {
        if (!canCreate) {
          setToast("Preencha nome, sobrenome e e-mail")
          setSaving(false)
          return
        }
        ...
        setToast("Perfil criado com sucesso")
      } else {
  ...
  async function removeExperience(id) {
    try { await deleteExperience(id); setExperiences((prev) => prev.filter((x) => x.id !== id)) }
    catch (err) { setToast(err.message) }
  }
  ...
  async function removeEducation(id) {
    try { await deleteEducation(id); setEducations((prev) => prev.filter((x) => x.id !== id)) }
    catch (err) { setToast(err.message) }
  }
    ...
    <main className="min-h-screen flex flex-col bg-slate-50">
      ...
      <section className="relative w-full overflow-hidden bg-gradient-to-br from-slate-900 via-slate-900 to-black text-white">
        ...
        <div className="relative max-w-4xl mx-auto px-6 py-16 flex flex-col md:flex-row md:items-end justify-between gap-8">
          ...
          <button
            onClick={handleSave}
            disabled={saving || loading || (isNew && !canCreate)}
          ...
          >
            ...
            {isNew ? (saving ? "Criando…" : "Criar perfil") : saving ? "Salvando…" : "Salvar perfil"}
          </button>
        </div>
      </section>

      {loading ? (
        <div className="flex-1 flex items-center justify-center py-32 text-slate-500">
        ...
        <div className="flex-1 w-full max-w-4xl mx-auto px-4 md:px-6 py-10 space-y-8">
          ...
          <section className="bg-white border border-slate-200 rounded-xl shadow-sm p-6 md:p-8 space-y-6">
            ...
            <div className="flex items-center gap-5">
              <div className="w-24 h-24 rounded-full overflow-hidden bg-slate-100 border border-slate-200 flex items-center justify-center text-slate-400 text-xl font-semibold shrink-0">
                {profile.avatar ? (
                  <img src={profile.avatar} alt="Foto do perfil" className="w-full h-full object-cover" />
                ) : (
                  initials || "FOTO"
                )}
              </div>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="sm:col-span-2">
                <Field label="Cargo / título profissional" value={profile.title} onChange={(e) => setField("title", e.target.value)} />
              </div>
              ...
              <div className="sm:col-span-2">
                <Field label="Portfólio / site" value={profile.portfolio} onChange={(e) => setField("portfolio", e.target.value)} />
              </div>
            </div>
          </section>
          ...
          {isNew && (
            <div className="bg-purple-50 border border-purple-100 rounded-xl p-5 text-sm text-purple-800">
              Crie seu perfil para liberar as seções de experiências, formação e competências.
            </div>
          )}
            ...
            <>
              ...
              <section className="bg-white border border-slate-200 rounded-xl shadow-sm p-6 md:p-8 space-y-6">
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={addExperience} className="inline-flex items-center gap-2 px-3 py-2 rounded-lg bg-slate-900 text-white text-sm hover:bg-black transition cursor-pointer">
                  ...
                  {experiences.map((exp) => (
                    <div key={exp.id} className="border border-slate-200 rounded-lg p-4 space-y-4 bg-slate-50/50">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <button onClick={() => removeExperience(exp.id)} className="inline-flex items-center gap-1 text-sm text-slate-500 hover:text-red-600 transition cursor-pointer">
              ...
              <section className="bg-white border border-slate-200 rounded-xl shadow-sm p-6 md:p-8 space-y-6">
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={addEducation} className="inline-flex items-center gap-2 px-3 py-2 rounded-lg bg-slate-900 text-white text-sm hover:bg-black transition cursor-pointer">
                    <Plus size={16} /> Adicionar
                  </button>
                </div>
                  ...
                  {educations.map((edu) => (
                    <div key={edu.id} className="border border-slate-200 rounded-lg p-4 space-y-4 bg-slate-50/50">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <button onClick={() => removeEducation(edu.id)} className="inline-flex items-center gap-1 text-sm text-slate-500 hover:text-red-600 transition cursor-pointer">
                          <Trash2 size={16} /> Remover
                        </button>
                      </div>
                    </div>
              ...
              <div className="flex justify-end">
                <button
                  onClick={handleSave}
                  disabled={saving}
                  className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-black text-white border border-black hover:bg-purple-600 hover:border-purple-600 transition-all duration-200 hover:-translate-y-0.5 active:translate-y-0 font-medium text-sm cursor-pointer disabled:opacity-60"
                >
                  {saving ? <Loader2 size={18} className="animate-spin" /> : <Save size={18} />}
