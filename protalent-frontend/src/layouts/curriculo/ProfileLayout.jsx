  ...
  return (
    <section className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
      ...
      <header className="flex items-center gap-6 p-8 bg-gradient-to-r from-slate-50 to-white">
        {/* Foto */}
        <div className="w-28 h-28 rounded-full overflow-hidden border border-slate-200 bg-slate-100 flex items-center justify-center text-slate-400">
          FOTO
        </div>
          ...
          <section className="space-y-3">
            ...
            <div className="flex flex-wrap gap-2">
              {data.habilidades.map(skill => (
                <span
                  key={skill}
                  className="px-3 py-1 text-xs rounded-full bg-slate-100 text-slate-700 border border-slate-200"
                >
                  {skill}
                </span>
              ))}
